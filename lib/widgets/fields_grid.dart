import 'package:flutter/material.dart';
import '../providers/crop.dart';
import './field_item.dart';
import 'package:provider/provider.dart';
import '../l10n/gallery_localizations.dart';
import '../providers/fields.dart';

import 'dart:ui';
/* 
enum BannerAction {
  reset,
  showMultipleActions,
  showLeading,
} */

class FieldsGrid extends StatelessWidget {
  final bool showFavs;

  FieldsGrid(this.showFavs);

  final noCropText = Text('No Fields Registered! Register Field');

  @override
  Widget build(BuildContext context) {
    final landsData = Provider.of<Fields>(context);
    final lands = landsData.items;
    if (landsData.items.length >0) {
      return GridView.builder(
        padding: const EdgeInsets.all(10.0),
        itemCount: lands.length,
        itemBuilder: (ctx, i) => ChangeNotifierProvider.value(
          value: lands[i],
          child: FieldItem(
            lands[i].id,
            lands[i].title,
            lands[i].imageUrl,
          ),
        ),
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
          crossAxisCount: 2,
          childAspectRatio: 3 / 2,
          mainAxisSpacing: 10,
          crossAxisSpacing: 10,
        ),
      );
    } else {
      return Banner();
    }

    
  }
}

class Banner extends StatefulWidget {
  const Banner();

  @override
  _BannerState createState() => _BannerState();
}

class _BannerState extends State<Banner> {
  
  var _showMultipleActions = true;
  var _displayBanner = true;
  var _showLeading = true;

/*   void handleDemoAction(BannerAction action) {
    setState(() {
      switch (action) {
        case BannerAction.reset:
          _displayBanner = true;
          
          _showLeading = true;
          break;
       
        case BannerAction.showLeading:
          _showLeading = !_showLeading;
          break;
      }
    });
  } */

  @override
  Widget build(BuildContext context) {
    
    return MaterialBanner(
      content: Text(GalleryLocalizations.of(context).noLandsText),
      leading: _showLeading
          ? CircleAvatar(
              child: Icon(Icons.bookmark),
              
            )
          : null,
      actions: [
       FlatButton(
          child: Text("Start adding lands here..."),
          onPressed: () {
            setState(() {
              _displayBanner = false;
             
              
             
            });
            
          },
        ),
      ],
     
    );

    
  }
}